...
<template>
  <div class="w-full max-w-4xl mx-auto space-y-6">
    ...
    <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 text-[13px] text-gray-700">
          <div>
            ...
            {{ format(user.firstName) }} {{ format(user.lastName) }}
          </div>

          <div>
            <span class="font-medium text-gray-900">Date of Birth:</span>
            {{ format(user.date_of_birth) }}
          </div>
          ...
          <div>
            ...
            {{ format(user.family_residence_location) }}
          </div>
    ...
    <div v-if="dependent?.length">
      <h2 class="!text-sm  text-gray-800 mb-4">👨‍👩‍👧 Dependent(s)</h2>
      <div v-for="(d, i) in dependent" :key="i" class="bg-gray-100 p-4 rounded-lg mb-2">
